...
// RequestDiffStat for request statictic
type RequestDiffStat struct {
	Request string
...
func (r *RequestDiffStat) Init(request string, stat *aggstatcmp.AggDiffStat) {
	r.Request = request

	r.Samples = sum(stat.SuccessCodes) + sum(stat.ErrorCodes)
	r.Errors = stat.Errors
...
func (r *RequestDiffStat) FillRowReq() []string {
	data := make([]string, 20)
	data[0] = r.Request
	data[1] = strconv.FormatFloat(r.Errors, 'f', 2, 64)

	data[2] = strconv.FormatFloat(r.ResponceTimeMean, 'f', 2, 64)
	data[3] = strconv.FormatFloat(r.ResponceTimeMin, 'f', 2, 64)
	data[4] = strconv.FormatFloat(r.ResponceTimeMax, 'f', 2, 64)
	data[5] = strconv.FormatFloat(r.ResponceTimeP90, 'f', 2, 64)
	data[6] = strconv.FormatFloat(r.ResponceTimeP95, 'f', 2, 64)
	data[7] = strconv.FormatFloat(r.ResponceTimeP99, 'f', 2, 64)

	data[8] = strconv.FormatFloat(r.SentMean, 'f', 2, 64)
	data[9] = strconv.FormatFloat(r.SentMin, 'f', 2, 64)
	data[10] = strconv.FormatFloat(r.SentMax, 'f', 2, 64)
	data[11] = strconv.FormatFloat(r.SentP90, 'f', 2, 64)
	data[12] = strconv.FormatFloat(r.SentP95, 'f', 2, 64)
	data[13] = strconv.FormatFloat(r.SentP99, 'f', 2, 64)

	data[14] = strconv.FormatFloat(r.ReceivedMean, 'f', 2, 64)
	data[15] = strconv.FormatFloat(r.ReceivedMin, 'f', 2, 64)
	data[16] = strconv.FormatFloat(r.ReceivedMax, 'f', 2, 64)
	data[17] = strconv.FormatFloat(r.ReceivedP90, 'f', 2, 64)
	data[18] = strconv.FormatFloat(r.ReceivedP95, 'f', 2, 64)
	data[19] = strconv.FormatFloat(r.ReceivedP99, 'f', 2, 64)

	return data
}

// RequestsDiffStat for requests statictic (in one label)
type RequestsDiffStat struct {
	Label string

	Stat    []RequestDiffStat
	ErrStat []ErrorDiffStat

	SumStat    RequestDiffStat
	ErrSumStat ErrorDiffStat
}

// SortRequests RequestsStat by column
func (r *RequestsDiffStat) SortRequests(sortCol SortColumn, sortDesc bool) {
...
func (r *RequestsDiffStat) SortErrors(sortCol int, sortDesc bool) {
...
// LabelDiffStat for requests statictic, grouped by labels
type LabelDiffStat struct {
	Started int64
	Ended   int64
	Name    string
	...
	Stat []RequestsDiffStat
}

// Init from aggstat.LabelURLAggDiffStat
func (l *LabelDiffStat) Init(lstat *aggstatcmp.LabelURLAggDiffStat, errors int) {
	l.Name = lstat.Name
	l.Started = lstat.Started
	l.Ended = lstat.Ended

	l.CmpName = lstat.CmpName
	l.CmpStarted = lstat.CmpStarted
	l.CmpEnded = lstat.CmpEnded

	l.Stat = make([]RequestsDiffStat, len(lstat.Stat))
	i := 0
	for label := range lstat.Stat {
		l.Stat[i].Label = label
		ustat := lstat.Stat[label]
		l.Stat[i].Stat = make([]RequestDiffStat, len(ustat.Stat))
		errStat := make([]ErrorDiffStat, len(ustat.Stat))
		j := 0
		e := 0
		for u := range ustat.Stat {
			l.Stat[i].Stat[j].Init(u, ustat.Stat[u])
			if len(ustat.Stat[u].ErrorCodes) > 0 {
				errStat[e].Init(u, ustat.Stat[u], l.Stat[i].Stat[j].Samples, errors)
				e++
			}
			j++
		}
		if e == j {
			...
			l.Stat[i].ErrStat = make([]ErrorDiffStat, e)
		...
		l.Stat[i].ErrSumStat.Init("Summary", &ustat.SumStat, l.Stat[i].SumStat.Samples, errors)
