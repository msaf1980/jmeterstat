...
function labelCmpQueriesTableRows(label, data, aggHeads) {
	let foots = []
	let rows = [];
	let columns1 = [];
	let columns2 = [];
	...
	columns1.push({text: "Executions", "colspan": 1});
	columns2.push("Errors %");
	foots.push(Math.round(100 * sumStat["Errors"]) / 100);
	// Responce Times
	columns1.push({text: "Responce Times (%)", "colspan": aggHeads.length})
	columns2.push.apply(columns2, aggHeads)
	for (let head of aggHeads) {
		foots.push(Math.round(100 *sumStat["Elapsed"][head]) / 100);
	}
	// Sent
	columns1.push({text: "Sent (%)", "colspan": aggHeads.length})
	columns2.push.apply(columns2, aggHeads)
	for (let head of aggHeads) {
		foots.push(Math.round(100 *sumStat["SentBytes"][head]) / 100);
	}
	// Received
	columns1.push({text: "Received (%)", "colspan": aggHeads.length})
	columns2.push.apply(columns2, aggHeads)
	for (let head of aggHeads) {
	...
	for (url in data["Stat"]) {
		...
		// Executions
		// Executions: Count, Errors %
		row.push(url, Math.round(100 * stat["Errors"]) / 100);
		// Responce Times
		for (let head of aggHeads) {
...
// second: rows[columns],
// third: array[queries][columns]
function labelCmpErrorsTableRows(label, data, maxErros) {
	let foots = []
	let rows = [];
	let columns = [];
	...
	for (i = 0; i < maxErros; i++) {
		if (i < errors.length) {
			foots.push(Math.round(100 * errors[i].Value) / 100);
	...
	for (url in data["Stat"]) {
		...
		for (i = 0; i < maxErros; i++) {
			if (i < errors.length) {
				row.push(Math.round(100 * errors[i].Value) / 100);
